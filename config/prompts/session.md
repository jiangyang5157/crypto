...
| `trend_intensity`| Signed `[-1, 1]`. Positive = Bullish trend, Negative = Bearish trend. `IS_TREND_STRONG` = Institutional backing. Prioritize shallow pullbacks in the trend direction (EXCEPT under dual confirmation — `IS_TREND_STRONG` AND `HAS_CVD_MOMENTUM` — where the Momentum Surge Entry is MANDATORY and Shallow Pullback DLEs are PROHIBITED). DO NOT execute counter-trend trades against an `IS_TREND_STRONG` direction. |
...
- **Momentum & Flow Riding**: If `IS_TREND_STRONG` AND `HAS_CVD_MOMENTUM` are BOTH TRUE, institutional backing is confirmed. You MUST execute a **Momentum Surge Entry** in the direction of the flow (per the **MOMENTUM SURGE EXEMPTION** below); **Shallow Pullback DLEs are PROHIBITED** in this exact state, because they create catastrophic non-fill risk in strong trends. If either condition is FALSE, directional momentum entries are PROHIBITED; you MAY consider fading entries (Exhaustion DLE or Sweep & Fade) if price-action confirms a clear reversal or exhaustion, or a **Shallow Pullback DLE** in the trend direction if `IS_TREND_STRONG` is TRUE. Do not force a **Momentum Surge Entry** without dual confirmation. All entries MUST still satisfy SHIELD LAW and **LIMIT ORDER PHYSICS**. Except under the **MOMENTUM SURGE EXEMPTION** (which exempts structural anchoring), the following is **MANDATORY**: Your `entry` MUST be anchored to a valid structural node (HVN/POC). If the nearest valid structure is further than `{max_entry_distance_atr}` ATR, you MUST NOT place a deep entry due to phantom order risk. Instead, either anchor your `entry` at the closest available structural boundary (including liquidation clusters or LVNs) within `{max_entry_distance_atr}` ATR, combined with a Dynamic Kinetic Shield for the stop, or defer to the Exhaustion Fading (DLE) or Sweep & Fade protocols. Under no circumstances should `entry` exceed `{max_entry_distance_atr}` ATR from `current_price`.
  ...
  IF `IS_TREND_STRONG` is TRUE AND `HAS_CVD_MOMENTUM` is TRUE simultaneously:
    ...
    → You MUST execute a Momentum Surge Entry with `entry` at `current_price` + 0.1 × `atr_micro` (BULLISH) or `current_price` − 0.1 × `atr_micro` (BEARISH) to guarantee a fill. Do NOT place `entry` beyond `current_price` in the pullback direction.
...
- `[GRAVITY_EXHAUSTION]`:
  ...
  ELSE IF institutional flow is confirmed (`IS_TREND_STRONG`) or flow aligns:
    → IF dual confirmation (`IS_TREND_STRONG` AND `HAS_CVD_MOMENTUM`): execute **Momentum Surge Entry** (per **MOMENTUM SURGE EXEMPTION**); Shallow Pullback DLE is PROHIBITED here.
...
- `[OPPORTUNITY_DENIAL]`: Execute **Momentum Entry** aligned with CVD or shallow **DLE**. **MANDATORY**: `entry` MUST be anchored to valid structure. Do not force a shallow entry in a vacuum just to satisfy proximity. Under dual confirmation (`IS_TREND_STRONG` AND `HAS_CVD_MOMENTUM`), execute a **Momentum Surge Entry** (per **MOMENTUM SURGE EXEMPTION**); Shallow Pullback DLE is PROHIBITED.
- `[TREND_STARVATION]`: Shift to shallow pullback or Momentum Entry. No deep DLEs. Under dual confirmation (`IS_TREND_STRONG` AND `HAS_CVD_MOMENTUM`), Momentum Surge Entry is MANDATORY (Shallow Pullback DLE PROHIBITED).
